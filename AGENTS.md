...
- Prefer [Taskfile](https://taskfile.dev/) for repeatable development, verification, and lifecycle commands.
- Prefer [`curl`](https://curl.se/) for HTTP request examples in Markdown documentation, and also provide companion `.http` files when practical for reproducible execution.
- Prefer [mise](https://mise.jdx.dev/) for runtime and tool version management.
...
- Prefer [process-compose](https://f1bonacc1.github.io/process-compose/) when an experiment requires multiple local processes.
- Prefer [pipx](https://pipx.pypa.io/stable/) for installing isolated Python CLI applications.
- Prefer [pnpm](https://pnpm.io/) for Node.js package management.
- Prefer [Lefthook](https://lefthook.dev/) for repository-managed Git hooks.
...
- Do not assume [`podman compose`](https://docs.podman.io/en/latest/markdown/podman-compose.1.html) uses `podman-compose`; it delegates to an external provider and may select `docker-compose` when both are installed.
